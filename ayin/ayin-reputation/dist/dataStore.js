...
class DataStore {
     ...
     * Store trade event
     */
    storeTradeEvent(event) {
        const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO trade_events
      (id, agent_id, market_id, direction, share_size, price, timestamp, block_number, tx_hash)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(event.id, event.agentId, event.marketId, event.direction, event.shareSize.toString(), event.price, event.timestamp, event.blockNumber, event.transactionHash);
    }
    /**
     * Store PnL event
     */
    storePnLEvent(event) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(event.id, event.agentId, event.marketId, event.winnings.toString(), event.loss.toString(), event.pnl.toString(), event.timestamp, event.blockNumber, event.transactionHash);
    }
    /**
    ...
    storeAgentTrade(trade) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(trade.id, trade.agentId, trade.marketId, trade.direction, trade.entrySize.toString(), trade.entryPrice, trade.entryTime, trade.outcome, trade.exitTime, trade.result, trade.pnl?.toString(), trade.returnPercent, trade.roiPercent, trade.timeInMarket, Math.floor(Date.now() / 1000));
    }
    /**
     * Close database
     */
