...
async function compatibleExtensions(coreBytecode, extensionBytecodes, chainId) {
  const chain = await chains.getChainByChainIdAsync(chainId);
  const rpcUrl = chains.getChainRPC(chain);
  ...
  const coreIface = new ethers.utils.Interface(index.coreContractAbi);
  ...
  const extensionIface = new ethers.utils.Interface(index.extensionContractAbi);
  ...
  return !index.hasDuplicates(selectors.flat(), (a, b) => a === b);
}
...
exports.compatibleExtensions = compatibleExtensions;
