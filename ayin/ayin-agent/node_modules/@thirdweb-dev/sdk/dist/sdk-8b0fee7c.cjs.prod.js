...
class ThirdwebSDK extends index.RPCConnectionHandler {
  ...
  async getContract(address, contractTypeOrABI) {
    ...
    if (!contractTypeOrABI || contractTypeOrABI === "custom") {
      try {
        ...
        newContract = await this.getContractFromAbi(resolvedAddress, await index.getCompositeABI(resolvedAddress, index.AbiSchema.parse(metadata.abi), this.getProvider(), this.options, this.storage));
      } catch (e) {
        // fallback to
        ...
        if (resolvedContractType && resolvedContractType !== "custom") {
          ...
          newContract = await this.getContractFromAbi(resolvedAddress, await index.getCompositeABI(resolvedAddress, index.AbiSchema.parse(contractAbi), this.getProvider(), this.options, this.storage));
        } else {
  ...
  async getContractFromAbi(address, abi) {
    const [resolvedAddress, {
      SmartContract
    }] = await Promise.all([index.resolveAddress(address), Promise.resolve().then(function () { return require('./smart-contract-dd0613cf.cjs.prod.js'); })]);
    if (this.contractCache.has(resolvedAddress)) {
      return this.contractCache.get(resolvedAddress);
    }
