...
import { aI as fetchPublishedContractFromPolygon, aJ as fetchAndCacheDeployMetadata, a1 as extractConstructorParamsFromAbi, dF as getRoyaltyEngineV1ByChainId, aH as THIRDWEB_DEPLOYER, a_ as getChainProvider, Y as fetchContractMetadata } from '../../../dist/index-33cd3415.browser.esm.js';
import { f as checkVerificationStatus, T as ThirdwebSDK, e as extractFunctionParamsFromAbi, t as convertParamValues } from '../../../dist/sdk-3e21fc5c.browser.esm.js';
...
import { f as fetchSourceFilesFromMetadata } from '../../../dist/fetchSourceFilesFromMetadata-906c835e.browser.esm.js';
import 'zod';
...
import '../../../dist/contract-publisher-01e4eba8.browser.esm.js';
import '@thirdweb-dev/contracts-js/dist/abis/ContractPublisher.json';
import '../../../dist/transactions-72f9603c.browser.esm.js';
import '@thirdweb-dev/contracts-js/dist/abis/TWMultichainRegistryLogic.json';
import '@thirdweb-dev/contracts-js/dist/abis/TWMultichainRegistryRouter.json';
import '../../../dist/fetchCurrencyValue-32d08b05.browser.esm.js';
import '../../../dist/normalizePriceValue-9851c0eb.browser.esm.js';
import '@thirdweb-dev/generated-abis';
import '../../../dist/deployContractDeterministic-7823c15c.browser.esm.js';
...
async function zkDeployContractDeterministic(signer, transaction, storage, metadataUri, options) {
  ...
  if (!contractDeployed) {
    ...
    try {
      const deployTx = await singleton.deploy(salt, transaction.bytecodeHash, transaction.constructorCalldata);
      await deployTx.wait();
...
async function zkDeployContractFromUri(publishMetadataUri, constructorParamValues, signer, storage, chainId, options, directDeployDeterministic, clientId, secretKey) {
  const {
    compilerMetadata,
    extendedMetadata
  ...
  if (extendedMetadata && extendedMetadata.factoryDeploymentData && !forceDirectDeploy && (!extendedMetadata.deployType || extendedMetadata.deployType !== "standard")) {
    ...
    if (directDeployDeterministic && !deterministicDeployment) {
      throw new Error("Deterministic deployment not supported for this contract.");
    } else if (directDeployDeterministic && deterministicDeployment) {
      const create2Factory = await zkDeployCreate2Factory(signer);
      const encodedArgs = utils.defaultAbiCoder.encode(constructorParamTypes, constructorParamValues);
      deployedAddress = zkComputeDeploymentAddress(bytecode, encodedArgs, create2Factory, options?.saltForProxyDeploy);
      if (await isZkContractDeployed(deployedAddress, signer.provider)) {
      ...
      try {
        await zkDeployContractDeterministic(signer, tx, storage, compilerMetadata.fetchedMetadataUri, options);
      } catch (e) {
        console.debug(`Error deploying contract at ${tx.predictedAddress}`, e?.message);
      }
    } else {
    ...
    if (deployedAddress) {
      // register on multichain registry
      await registerContractOnMultiChainRegistry(deployedAddress, chainId, compilerMetadata.fetchedMetadataUri);
    }
  }
