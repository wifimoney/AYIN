...
class SimpleStrategy {
    constructor(marketFetcher, logger) {
        this.YES_THRESHOLD = 60; // 60% confidence to trade YES
        this.NO_THRESHOLD = 40; // 40% or less to trade NO
    ...
    async run(context) {
        ...
        for (const market of context.markets) {
            ...
            if (yesProbability > this.YES_THRESHOLD) {
                return {
                    signal: this.createTradeSignal(market.marketId, types_1.TradeDirection.YES, yesProbability, context),
            ...
            if (yesProbability < this.NO_THRESHOLD) {
                return {
                    signal: this.createTradeSignal(market.marketId, types_1.TradeDirection.NO, 100 - yesProbability, context),
                    nextCheckTime: context.timestamp + context.mandate.expiryTime,
                };
            }
        }
     ...
     */
    createTradeSignal(marketId, direction, confidence, context) {
        ...
        return {
            ...
            reasoning: direction === types_1.TradeDirection.YES
